...
public class RedisLimiterManager {
    ...
    public boolean doRateLimit(String key) {
        ...
        boolean trySetRate = rateLimiter.trySetRate(RateType.OVERALL, 4, 1  ,RateIntervalUnit.SECONDS);
        if (trySetRate) {
    ...
    public boolean doRateLimitAI(String key) {
        ...
        // 参数3 rateInterval：限流时间间隔，即限流速率的计算周期长度。
        // 参数4 unit：限流时间间隔单位，可以是秒、毫秒等。
        boolean trySetRate = rateLimiter.trySetRate(RateType.OVERALL, 2, 1, RateIntervalUnit.DAYS);
        if (trySetRate) {
            log.info("init rate = {}, interval = {}", rateLimiter.getConfig().getRate(), rateLimiter.getConfig().getRateInterval());
        }
        // 每当一个操作来了后，请求一个令牌
